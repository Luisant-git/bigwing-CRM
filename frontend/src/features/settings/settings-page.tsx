  ...
  return (
    <div>
      ...
      <div className="flex flex-col gap-6 lg:flex-row">
        ...
        {/* Content */}
        <div className={`flex-1 ${!isMobileDetail ? "hidden lg:block" : "block"}`}>
          {LOOKUP_SECTIONS.map((s) =>
            activeSection === s.key ? (
              <LookupEditor key={s.key} label={s.label} apiName={s.key} description={s.description} onBack={() => setIsMobileDetail(false)} />
            ) : null
          )}
        </div>
      </div>
...
function LookupEditor({ label, apiName, description, onBack }: { label: string; apiName: string; description: string; onBack: () => void }) {
                ...
                >
                  ...
                  <div className="flex shrink-0 items-center gap-2">
                    <button
                      ...
                      className={`flex items-center gap-1 rounded-md px-2 py-1 text-[11px] font-medium whitespace-nowrap ${item.isActive ? "bg-green-50 text-green-600" : "bg-red-50 text-red-500"}`}
    ...
    >
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        <button
          type="button"
          onClick={onCancel}
          className="flex items-center gap-1 rounded-md border border-gray-300 bg-white px-3 py-1.5 text-[11px] font-medium text-gray-500 hover:bg-gray-50"
        >
          <X size={12} /> Cancel
        </button>
      </div>
